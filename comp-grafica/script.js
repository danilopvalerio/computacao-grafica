...
function clearCanvasAndDrawBase() {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    let p1_device = ndcToDevice(worldToNDC(0, world.ymin).x, worldToNDC(0, world.ymin).y);
    let p2_device = ndcToDevice(worldToNDC(0, world.ymax).x, worldToNDC(0, world.ymax).y);
    ctx.moveTo(p1_device.x, p1_device.y);
    ctx.lineTo(p2_device.x, p2_device.y);
    p1_device = ndcToDevice(worldToNDC(world.xmin, 0).x, worldToNDC(world.xmin, 0).y);
    p2_device = ndcToDevice(worldToNDC(world.xmax, 0).x, worldToNDC(world.xmax, 0).y);
    ctx.moveTo(p1_device.x, p1_device.y);
    ctx.lineTo(p2_device.x, p2_device.y);
    ctx.stroke();
    ...
    if (document.getElementById("viewport_toggle").checked) {
        ...
        ctx.strokeRect(viewport.xvmin, viewport.yvmin, viewport.xvmax - viewport.xvmin, viewport.yvmax - viewport.yvmin);
    }
}
...
function changeFigure() {
  ...
  [
    "lineParams", "circleParams", "ellipseParams", "cubeParams", "quadParams", "img-pgmParams",
  ].forEach((id) => {
...
function createPolygonObject() {
    ...
    if (figure === 'cube') {
        ...
        points = [
            [-halfSize, -halfSize, 1], [halfSize, -halfSize, 1],
            [halfSize, halfSize, 1],   [-halfSize, halfSize, 1],
        ];
    } else { // quad
        const A = getPoint("A"), B = getPoint("B"), C = getPoint("C"), D = getPoint("D");
        if (!A || !B || !C || !D) {
            alert("Coordenadas do quadrilátero inválidas.");
            return;
        }
        points = [[A.x, A.y, 1], [B.x, B.y, 1], [C.x, C.y, 1], [D.x, D.y, 1]];
    }
...
function redrawScene(isAnimation = false) {
  ...
  // MODIFICADO: Só limpa o log de iterações se NÃO for uma animação
  if (!isAnimation) {
    clearLog();
  }
  ...
  const showLogPanel = ["line", "circle", "ellipse", "polygon"].includes(activeObject.type);
  document.getElementById("bresenhamParams").style.display = showLogPanel ? "block" : "none";

  switch(activeObject.type) {
    case 'line':
      if (activeObject.alg === 'dda') {
        LineDDA(activeObject.x1, activeObject.y1, activeObject.x2, activeObject.y2);
      } else {
        LineBresenham(activeObject.x1, activeObject.y1, activeObject.x2, activeObject.y2);
      }
      break;
    case 'circle':
      if (activeObject.alg === 'midpoint') {
        CircleMidpoint(activeObject.cx, activeObject.cy, activeObject.radius);
      } else if (activeObject.alg === 'equation') {
        CircleEquation(activeObject.cx, activeObject.cy, activeObject.radius);
      } else {
        CircleTrigonometric(activeObject.cx, activeObject.cy, activeObject.radius);
      }
      break;
    case 'ellipse':
      EllipseMidpoint(activeObject.cx, activeObject.cy, activeObject.rx, activeObject.ry);
      break;
    case 'polygon':
      const isPgm = activeObject.isPgm;
      for (let i = 0; i < activeObject.points.length; i++) {
          if (isPgm) {
              setPixelWorld(activeObject.points[i][0], activeObject.points[i][1]);
          } else {
              const next = (i + 1) % activeObject.points.length;
              LineBresenham(
                  activeObject.points[i][0], activeObject.points[i][1],
                  activeObject.points[next][0], activeObject.points[next][1]
              );
          }
      }
      break;
...
function applyTranslation() {
  ...
  if (!activeObject) { alert("Desenhe a forma primeiro."); return; }

  switch(activeObject.type) {
    case 'polygon':
      activeObject.points = activeObject.points.map(([x, y, z]) => [x + tx, y + ty, z]);
      break;
    case 'line':
      activeObject.x1 += tx; activeObject.y1 += ty;
      activeObject.x2 += tx; activeObject.y2 += ty;
      break;
    case 'circle':
    case 'ellipse':
      activeObject.cx += tx; activeObject.cy += ty;
      break;
  }
...
function applyScale() {
  ...
  switch(activeObject.type) {
      case 'polygon':
          ...
          break;
      case 'line':
          center = { x: (activeObject.x1 + activeObject.x2)/2, y: (activeObject.y1 + activeObject.y2)/2 };
...
function applyRotation(isAnimation = false) {
  ...
  switch(activeObject.type) {
    ...
    case 'line':
      center = { x: (activeObject.x1 + activeObject.x2)/2, y: (activeObject.y1 + activeObject.y2)/2 };
      const p1 = rotatePoint({ x: activeObject.x1, y: activeObject.y1 }, center);
      const p2 = rotatePoint({ x: activeObject.x2, y: activeObject.y2 }, center);
      activeObject.x1 = p1.x; activeObject.y1 = p1.y;
      activeObject.x2 = p2.x; activeObject.y2 = p2.y;
      break;
    case 'circle':
    case 'ellipse':
      break;
...
function applyShear() {
  ...
  const shx = document.getElementById("shx").disabled ? 0 : parseFloat(document.getElementById("shx").value);
  const shy = document.getElementById("shy").disabled ? 0 : parseFloat(document.getElementById("shy").value);
  ...
  switch(activeObject.type) {
      ...
      case 'circle':
      case 'ellipse':
          alert("Cisalhamento não é suportado para círculos ou elipses nesta implementação.");
          break;
  }
  ...
  function readToken() {
    ...
    while (currentIndex < view.length) {
      ...
      if (/\s/.test(String.fromCharCode(charCode))) {
        if (token.length > 0) { currentIndex++; return token; }
      } else if (String.fromCharCode(charCode) === "#") {
        while (currentIndex < view.length && String.fromCharCode(view[currentIndex]) !== "\n") { currentIndex++; }
      } else { token += String.fromCharCode(charCode); }
      currentIndex++;
    } return token;
  }

  const magic = readToken();
  if (magic !== "P2" && magic !== "P5") { alert("Erro: Formato de arquivo PGM inválido."); return null; }
  const width = parseInt(readToken());
  const height = parseInt(readToken());
  const maxVal = parseInt(readToken());
  if (isNaN(width) || isNaN(height) || isNaN(maxVal)) { alert("Erro ao ler o cabeçalho do PGM."); return null; }
  let pixels;
  if (magic === "P5") { pixels = view.subarray(currentIndex); }
  else { const textDecoder = new TextDecoder(); const remainingText = textDecoder.decode(view.subarray(currentIndex)); pixels = remainingText.split(/\s+/).filter((s) => s).map((s) => parseInt(s)); }
  if (pixels.length < width * height) { alert(`Erro: O arquivo PGM está incompleto.`); return null; }
...
function loadAndDrawPGM() {
  ...
  if (fileInput.files.length === 0) { alert("Por favor, selecione um arquivo PGM."); return; }
  const file = fileInput.files[0];
  const reader = new FileReader();
  reader.onload = function (e) {
    const pgmData = parsePGM(e.target.result);
    if (!pgmData) { activeObject = null; redrawScene(); return; }
...
function setupMouseCoordsTracker() {
  ...
  canvas.addEventListener("mousemove", (event) => {
    ...
    mouseCoordsElement.textContent = `Coordenadas do Mundo: (${worldCoords.x.toFixed(2)}, ${worldCoords.y.toFixed(2)})`;
  });
}
...
function logIteration(x, y, d = null, k = null) {
  ...
  if (!logContainer.querySelector("table")) {
    ...
    table.innerHTML = `<thead><tr><th>${d !== null ? "d" : "k"}</th><th>X (mundo)</th><th>Y (mundo)</th></tr></thead><tbody></tbody>`;
    logContainer.appendChild(table);
  }
  const tbody = logContainer.querySelector("tbody");
  const row = document.createElement("tr");
  const d_val = d !== null && typeof d === "number" ? d.toFixed(2) : (k !== null ? k : "-");
